...
router.route("/update/:id").put(async (req, res) => {
  ...
  const { First_name, Last_name, NIC, Address, Contact, Email, Designation } =
    req.body;
  const updateUser = {
    First_name,
    Last_name,
  ...
  const update = await EmployeeModel.findByIdAndUpdate(userId, updateUser)
    .then(() => {
...
router.route("/get/:id").get((req, res) => {
  ...
  EmployeeModel.find({ First_name: id })
    .then((user) => {
      res.json(user);
    })
    .catch((err) => {
