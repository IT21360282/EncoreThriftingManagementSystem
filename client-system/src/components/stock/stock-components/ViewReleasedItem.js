...
export default function ViewReleasedItem() {
    const {id} = useParams()
  return (
    <div>
        {id}
      <ViewReleasedItemBody id = {id}/>
    </div>
  )
}
...
class ViewReleasedItemBody extends Component {
    constructor(props){
        super(props);
    ...
    componentDidMount(){
        ...
        axios.get(`http://localhost:8000/releaseditem/get/${this.state.id}`).then(res=>{
            if(res.data.success){
                this.setState({
