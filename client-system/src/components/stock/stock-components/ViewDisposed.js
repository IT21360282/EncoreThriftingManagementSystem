...
export default function ViewDisposed() {
    const {id} = useParams()
  return (
    <div>
        {id}
      <ViewDisposedBody id = {id}/>
    </div>
  )
}
...
class ViewDisposedBody extends Component {
    constructor(props){
        super(props);
    ...
    componentDidMount(){
        ...
        axios.get(`http://localhost:8000/disposeditem/get/${this.state.id}`).then(res=>{
            if(res.data.success){
                this.setState({
