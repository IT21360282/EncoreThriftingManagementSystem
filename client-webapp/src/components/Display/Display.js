...
//display items in display view page
    const getItems = () => {
      ...
      const handleSearch = (searchQuery) => {
        axios.get(`http://localhost:8000/display/searchItems?query=${searchQuery}`)
          .then((res) => {
            setItems(res.data);
          })
          .catch((err) => console.log(err));
  ...
  return (
    <div className='displayBox'>
      ...
      <div className="Category">
        <h1 id='categoryTitle'>Books</h1>

    <div className="container">
        {items
        .filter((item) => item.pCategory === "Books")
        .slice(0,5)
        .map((item) => (
          <li key={item._id}>
               <div className="categoryItem">
               <img src={item.pImageURL} alt={item.pImageURL} style={{width:"200px"}} className="categoryImg" />
                <div className="itemName" style={{fontSize:"18px", fontFamily:"Verdana, Geneva, Tahoma, sans-serif"}}><p>{item.pName.slice(0, 18)}...</p></div>
                <div className="description"><p>{item.pDescription.slice(0, 20)}...</p></div>
        ...
        <h1>Electronics & Electricals</h1>


        <div className="container">

                {items
                .filter((item) => item.pCategory === "Electronics")
                .slice(0,5)
                .map((item) => (
                <li key={item._id}>
                    <div className="categoryItem">
        ...
        <div className="container">
            ...
            {items
            .filter((item) => item.pCategory === "Furniture")
            .slice(0,5)
            .map((item) => (
            <li key={item._id}>
                <div className="categoryItem">
        ...
        <div className="container">
                ...
                {items
                .filter((item) => item.pCategory === "Tools")
                .slice(0,5)
                .map((item) => (
                <li key={item._id}>
                    <div className="categoryItem">
        ...
        <div className="container">
                ...
                {items
                .filter((item) => item.pCategory === "Toys")
                .slice(0,5)
                .map((item) => (
                <li key={item._id}>
                    <div className="categoryItem">
